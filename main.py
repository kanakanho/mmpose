...
def run_inference(args: argparse.Namespace) -> None:
    ...
    for idx in camera_indices:
        ...
        caps[idx] = cap
    print(f"[Main] カメラ {camera_indices} を起動しました。")
    ...
    try:
        while True:
            ...
            if use_3d:
                ...
                if res0 is not None:
                    ...
                    kps3d_dummy = np.concatenate(
                        [kps2d, np.zeros((kps2d.shape[0], 1), dtype=np.float32)], axis=1
                    )
            ...
            if preview is not None:
                frames_list = [frames[idx] for idx in camera_indices if idx in frames]
